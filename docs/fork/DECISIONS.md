...
- **2026-09-25 D48 (estate reply):** Patrick requested the existing estate-aware read-email/draft/push-to-Gmail workflow inside Skim. One fork-owned SSH action invokes the existing enrolled Codex profile on hel-work; no new model service or context database. Investigation is read-only; a host adapter creates through os-google and Skim imports the verified Gmail draft. The gateway has no draft-update tool, so first release generates once and uses normal Skim editing. This user-requested remote drafting operation is deliberately online and receipt-backed rather than replayed through the IMAP mutation queue: replaying an ambiguous create could duplicate mail drafts.

- **2026-09-25 D49 (visible scope and documentation):** The estate-reply delivery adds Draft reply inside the reading pane and preserves the existing inbox/sidebar. On me and Waiting remain installed despite Patrick's feedback that they are not useful. Their replacement and the broader research proposals remain open work, not delivered features. Patrick confirmed the location/scope explanation and requested documentation updates; usage is in ESTATE-REPLY-PLAN.md and current delivery status remains canonical in STATUS.md.
